...
mod into_row;
use crate::tds_core::tds::codec::encode::Encode;
use crate::tds_core::{tds::codec::ColumnData, BytesMutWithTypeInfo, SqlReadBytes, TokenType};
use bytes::BufMut;
...
impl<'a> Encode<BytesMutWithDataColumns<'a>> for TokenRow<'a> {
    fn encode(self, dst: &mut BytesMutWithDataColumns<'a>) -> crate::tds_core::Result<()> {
        dst.put_u8(TokenType::Row as u8);

        if self.data.len() != dst.data_columns().len() {
            return Err(crate::tds_core::Error::BulkInput(
                format!(
...
impl TokenRow<'static> {
    ...
    /// based on that.
    pub(crate) async fn decode<R>(src: &mut R) -> crate::tds_core::Result<Self>
    where
    ...
    pub(crate) async fn decode_nbc<R>(src: &mut R) -> crate::tds_core::Result<Self>
    where
        R: SqlReadBytes + Unpin,
    {
        let col_meta = src.context().last_meta().unwrap();
...
impl RowBitmap {
    ...
    /// type is `NbcRowToken`.
    async fn decode<R>(src: &mut R, columns: usize) -> crate::tds_core::Result<Self>
    where
        R: SqlReadBytes + Unpin,
    {
...
mod tests {
    use super::*;
    use crate::tds_core::{BaseMetaDataColumn, ColumnFlag, FixedLenType, MetaDataColumn, TypeInfo};
    use bytes::BytesMut;
