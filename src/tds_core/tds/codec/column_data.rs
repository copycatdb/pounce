...
use crate::tds_core::tds::time::{Date, DateTime2, DateTimeOffset, Time};
use crate::tds_core::{
...
impl<'a> ColumnData<'a> {
    ...
    pub(crate) async fn decode<R>(src: &mut R, ctx: &TypeInfo) -> crate::tds_core::Result<ColumnData<'a>>
    where
...
impl<'a> Encode<BytesMutWithTypeInfo<'a>> for ColumnData<'a> {
    fn encode(self, dst: &mut BytesMutWithTypeInfo<'a>) -> crate::tds_core::Result<()> {
                    ...
                    || vlc.r#type() == VarLenType::BigVarChar =>
            {
                if let Some(str) = opt {
                    ...
                    if let encoding_rs::EncoderResult::Unmappable(_) = res {
                        return Err(crate::tds_core::Error::Encoding("unrepresentable character".into()));
                    }
...
mod tests {
    use super::*;
    use crate::tds_core::sql_read_bytes::test_utils::IntoSqlReadBytes;
    use crate::tds_core::tds::Collation;
    use crate::tds_core::{Error, VarLenContext};
    use bytes::BytesMut;
