use crate::tds_core::tds::stream::ReceivedToken;
use crate::tds_core::tds::codec::ColumnFlag;
use crate::tds_core::tds::codec::TokenInfo;
use crate::tds_core::{row::ColumnType, Column, Row};
use futures_util::{
    ready,
...
///
/// ```
/// # use crate::tds_core::{Config, QueryItem};
...
/// # let mut client = crate::tds_core::Client::connect(config, tcp.compat_write()).await?;
...
pub struct QueryStream<'a> {
    token_stream: Peekable<BoxStream<'a, crate::tds_core::Result<ReceivedToken>>>,
    columns: Option<Arc<Vec<Column>>>,
    result_set_index: Option<usize>,
    info_messages: Vec<TokenInfo>,
}
...
impl<'a> Debug for QueryStream<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QueryStream")
            .field(
                "token_stream",
                &"BoxStream<'a, crate::tds_core::Result<ReceivedToken>>",
            )
            .finish()
    }
}
...
impl<'a> QueryStream<'a> {
    pub(crate) fn new(token_stream: BoxStream<'a, crate::tds_core::Result<ReceivedToken>>) -> Self {
        Self {
    ...
    /// error.
    pub(crate) async fn forward_to_metadata(&mut self) -> crate::tds_core::Result<()> {
        loop {
            ...
            match item {
                ...
                Some(ReceivedToken::Info(_)) => {
                    ...
                    if let Some(Ok(ReceivedToken::Info(info))) = self.token_stream.try_next().await.transpose() {
    ...
    /// # }
    /// ```
    pub async fn columns(&mut self) -> crate::tds_core::Result<Option<&[Column]>> {
        use ReceivedToken::*;
    ...
    /// of querying.
    pub async fn into_results(mut self) -> crate::tds_core::Result<Vec<Vec<Row>>> {
        ...
        while let Some(item) = self.try_next().await? {
            match (item, &mut result) {
                ...
                (QueryItem::Message(_), _) => { /* skip info messages */ }
            }
        }
    ...
    /// results.
    pub async fn into_first_result(self) -> crate::tds_core::Result<Vec<Row>> {
    ...
    /// further rows.
    pub async fn into_row(self) -> crate::tds_core::Result<Option<Row>> {
    ...
    pub fn into_row_stream(self) -> BoxStream<'a, crate::tds_core::Result<Row>> {
...
impl<'a> Stream for QueryStream<'a> {
    type Item = crate::tds_core::Result<QueryItem>;
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if !this.info_messages.is_empty() {
            ...
            return Poll::Ready(Some(Ok(QueryItem::Message(info))));
        }
        ...
        loop {
            ...
            return match token {
                ReceivedToken::NewResultset(meta) => {
                    let column_meta = meta
                        ...
                        .map(|x| Column {
                            ...
                            nullable: Some(x.base.flags.contains(ColumnFlag::Nullable)),
                        })
                        .collect::<Vec<_>>();
                ...
                ReceivedToken::Info(info) => {
                    Poll::Ready(Some(Ok(QueryItem::Message(info))))
                }
                _ => continue,
            };
        }
