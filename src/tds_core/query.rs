...
impl<'a> Query<'a> {
    ...
    ///
    /// ```no_run
    /// # use crate::tds_core::{Config, Query};
    ...
    pub async fn execute<'b, S>(self, client: &'b mut Client<S>) -> crate::tds_core::Result<ExecuteResult>
    where
    ...
    pub async fn query<'b, S>(self, client: &'b mut Client<S>) -> crate::tds_core::Result<QueryStream<'b>>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
        client.connection.flush_stream().await?;
