...
use crate::tds_core::client::Config;
...
pub trait SqlBrowser {
    ...
    async fn connect_named(builder: &Config) -> crate::tds_core::Result<Self>
    where
...
fn get_port_from_sql_browser_reply(
    ...
    len: usize,
    instance_name: &str,
) -> crate::tds_core::Result<u16> {
    const DELIMITER: &[u8] = b"tcp;";

    buf.truncate(len);

    let err = crate::tds_core::Error::Conversion(
