...
use crate::tds_core::client::{tls::TlsPreloginWrapper, tls_stream::create_tls_stream};
use crate::tds_core::{
...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Connection<S> {
    ...
    pub(crate) async fn connect(config: Config, tcp_stream: S) -> crate::tds_core::Result<Connection<S>> {
        let context = {
    ...
    async fn flush_done(&mut self) -> crate::tds_core::Result<TokenDone> {
        TokenStream::new(self).flush_done().await
    }

    #[cfg(any(windows, feature = "integrated-auth-gssapi"))]
    /// Flush the incoming token stream until receiving `SSPI` token.
    async fn flush_sspi(&mut self) -> crate::tds_core::Result<TokenSspi> {
    ...
    pub async fn send<E>(&mut self, mut header: PacketHeader, item: E) -> crate::tds_core::Result<()>
    where
    ...
    pub(crate) async fn flush_sink(&mut self) -> crate::tds_core::Result<()> {
    ...
    pub async fn flush_stream(&mut self) -> crate::tds_core::Result<()> {
        self.buf.truncate(0);
    ...
    ) -> crate::tds_core::Result<PreloginMessage> {
    ...
    async fn tls_handshake(self, _: &Config, _: EncryptionLevel) -> crate::tds_core::Result<Self> {
        event!(
            Level::WARN,
            "TLS encryption is not enabled. All traffic including the login credentials are not encrypted."
        );
    ...
    pub(crate) async fn close(mut self) -> crate::tds_core::Result<()> {
        self.transport.close().await
    }
}

impl<S: AsyncRead + AsyncWrite + Unpin + Send> Stream for Connection<S> {
    type Item = crate::tds_core::Result<Packet>;
