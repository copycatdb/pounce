...
macro_rules! to_sql {
    ($target:ident, $( $ty:ty: ($variant:expr, $val:expr) ;)* ) => {
        $(
            impl crate::tds_core::ToSql for $ty {
                fn to_sql(&self) -> crate::tds_core::tds::codec::ColumnData<'_> {
                    let $target = self;
                    $variant(Some($val))
                }
            }

            impl crate::tds_core::ToSql for Option<$ty> {
                fn to_sql(&self) -> crate::tds_core::tds::codec::ColumnData<'_> {
                    match self {
                        Some(val) => {
                            let $target = val;
...
macro_rules! into_sql {
    ($target:ident, $( $ty:ty: ($variant:expr, $val:expr) ;)* ) => {
        $(
            impl<'a> crate::tds_core::IntoSql<'a> for $ty {
                fn into_sql(self) -> crate::tds_core::tds::codec::ColumnData<'a> {
                    let $target = self;
                    $variant(Some($val))
                }
            }

            impl<'a> crate::tds_core::IntoSql<'a> for Option<$ty> {
                fn into_sql(self) -> crate::tds_core::tds::codec::ColumnData<'a> {
                    match self {
                        Some(val) => {
                            let $target = val;
...
macro_rules! from_sql {
    ($( $ty:ty: $($pat:pat => ($borrowed_val:expr, $owned_val:expr)),* );* ) => {
        $(
            impl<'a> crate::tds_core::FromSql<'a> for $ty {
                fn from_sql(data: &'a crate::tds_core::tds::codec::ColumnData<'static>) -> crate::tds_core::Result<Option<Self>> {
                    match data {
                        $( $pat => Ok($borrowed_val), )*
                        _ => Err(crate::tds_core::Error::Conversion(format!("cannot interpret {:?} as an {} value", data, stringify!($ty)).into()))
                    }
                }
            }

            impl crate::tds_core::FromSqlOwned for $ty {
                fn from_sql_owned(data: crate::tds_core::tds::codec::ColumnData<'static>) -> crate::tds_core::Result<Option<Self>> {
                    match data {
                        $( $pat => Ok($owned_val), )*
                        _ => Err(crate::tds_core::Error::Conversion(format!("cannot interpret {:?} as an {} value", data, stringify!($ty)).into()))
                    }
