...
use crate::tabby::tds::time::{Date, DateTime2, DateTimeOffset, Time};
use crate::tabby::{
...
impl<'a> ColumnData<'a> {
    ...
    pub(crate) async fn decode<R>(src: &mut R, ctx: &TypeInfo) -> crate::tabby::Result<ColumnData<'a>>
    where
...
impl<'a> Encode<BytesMutWithTypeInfo<'a>> for ColumnData<'a> {
    fn encode(self, dst: &mut BytesMutWithTypeInfo<'a>) -> crate::tabby::Result<()> {
                    ...
                    || vlc.r#type() == VarLenType::BigVarChar =>
            {
                if let Some(str) = opt {
                    ...
                    if let encoding_rs::EncoderResult::Unmappable(_) = res {
                        return Err(crate::tabby::Error::Encoding("unrepresentable character".into()));
                    }
...
mod tests {
    use super::*;
    use crate::tabby::sql_read_bytes::test_utils::IntoSqlReadBytes;
    use crate::tabby::tds::Collation;
    use crate::tabby::{Error, VarLenContext};
    use bytes::BytesMut;
