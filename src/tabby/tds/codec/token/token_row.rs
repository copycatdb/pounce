...
mod into_row;
use crate::tabby::tds::codec::encode::Encode;
use crate::tabby::{tds::codec::ColumnData, BytesMutWithTypeInfo, SqlReadBytes, TokenType};
use bytes::BufMut;
...
impl<'a> Encode<BytesMutWithDataColumns<'a>> for TokenRow<'a> {
    fn encode(self, dst: &mut BytesMutWithDataColumns<'a>) -> crate::tabby::Result<()> {
        dst.put_u8(TokenType::Row as u8);

        if self.data.len() != dst.data_columns().len() {
            return Err(crate::tabby::Error::BulkInput(
                format!(
...
impl TokenRow<'static> {
    ...
    /// based on that.
    pub(crate) async fn decode<R>(src: &mut R) -> crate::tabby::Result<Self>
    where
    ...
    pub(crate) async fn decode_nbc<R>(src: &mut R) -> crate::tabby::Result<Self>
    where
        R: SqlReadBytes + Unpin,
    {
        let col_meta = src.context().last_meta().unwrap();
...
impl RowBitmap {
    ...
    /// type is `NbcRowToken`.
    async fn decode<R>(src: &mut R, columns: usize) -> crate::tabby::Result<Self>
    where
        R: SqlReadBytes + Unpin,
    {
...
mod tests {
    use super::*;
    use crate::tabby::{BaseMetaDataColumn, ColumnFlag, FixedLenType, MetaDataColumn, TypeInfo};
    use bytes::BytesMut;
