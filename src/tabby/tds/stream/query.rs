use crate::tabby::tds::stream::ReceivedToken;
use crate::tabby::tds::codec::ColumnFlag;
use crate::tabby::tds::codec::TokenInfo;
use crate::tabby::{row::ColumnType, Column, Row};
use futures_util::{
    ready,
...
///
/// ```
/// # use crate::tabby::{Config, QueryItem};
...
/// # let mut client = crate::tabby::Client::connect(config, tcp.compat_write()).await?;
...
pub struct QueryStream<'a> {
    token_stream: Peekable<BoxStream<'a, crate::tabby::Result<ReceivedToken>>>,
...
impl<'a> Debug for QueryStream<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QueryStream")
            .field(
                "token_stream",
                &"BoxStream<'a, crate::tabby::Result<ReceivedToken>>",
            )
            .finish()
    }
}
...
impl<'a> QueryStream<'a> {
    pub(crate) fn new(token_stream: BoxStream<'a, crate::tabby::Result<ReceivedToken>>) -> Self {
        Self {
    ...
    /// error.
    pub(crate) async fn forward_to_metadata(&mut self) -> crate::tabby::Result<()> {
        loop {
    ...
    /// # }
    /// ```
    pub async fn columns(&mut self) -> crate::tabby::Result<Option<&[Column]>> {
        use ReceivedToken::*;
    ...
    /// of querying.
    pub async fn into_results(mut self) -> crate::tabby::Result<Vec<Vec<Row>>> {
    ...
    /// results.
    pub async fn into_first_result(self) -> crate::tabby::Result<Vec<Row>> {
    ...
    /// further rows.
    pub async fn into_row(self) -> crate::tabby::Result<Option<Row>> {
    ...
    pub fn into_row_stream(self) -> BoxStream<'a, crate::tabby::Result<Row>> {
...
impl<'a> Stream for QueryStream<'a> {
    type Item = crate::tabby::Result<QueryItem>;
