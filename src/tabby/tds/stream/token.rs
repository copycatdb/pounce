use crate::tabby::tds::codec::TokenSspi;
use crate::tabby::{
    client::Connection,
    tds::codec::{
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    pub(crate) async fn flush_done(self) -> crate::tabby::Result<TokenDone> {
        ...
        loop {
            match stream.try_next().await? {
                ...
                Some(_) => (),
                None => return Err(crate::tabby::Error::Protocol("Never got DONE token.".into())),
            }
        }
    }

    #[cfg(any(windows, feature = "integrated-auth-gssapi"))]
    pub(crate) async fn flush_sspi(self) -> crate::tabby::Result<TokenSspi> {
        let mut stream = self.try_unfold();
        let mut last_error = None;
        ...
        loop {
            match stream.try_next().await? {
                ...
                None => match last_error {
                    Some(err) => return Err(crate::tabby::Error::Server(err)),
                    None => return Err(crate::tabby::Error::Protocol("Never got SSPI token.".into())),
                },
            }
        }
    }

    async fn get_col_metadata(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_row(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_nbc_row(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_return_value(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let return_value = TokenReturnValue::decode(self.conn).await?;
        event!(Level::TRACE, message = ?return_value);
        Ok(ReceivedToken::ReturnValue(return_value))
    }

    async fn get_return_status(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let status = self.conn.read_u32_le().await?;
        Ok(ReceivedToken::ReturnStatus(status))
    }

    async fn get_error(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_order(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let order = TokenOrder::decode(self.conn).await?;
        event!(Level::TRACE, message = ?order);
        Ok(ReceivedToken::Order(order))
    }

    async fn get_done_value(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let done = TokenDone::decode(self.conn).await?;
        event!(Level::TRACE, "{}", done);
        Ok(ReceivedToken::Done(done))
    }

    async fn get_done_proc_value(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let done = TokenDone::decode(self.conn).await?;
        event!(Level::TRACE, "{}", done);
        Ok(ReceivedToken::DoneProc(done))
    }

    async fn get_done_in_proc_value(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let done = TokenDone::decode(self.conn).await?;
        event!(Level::TRACE, "{}", done);
        Ok(ReceivedToken::DoneInProc(done))
    }

    async fn get_env_change(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_info(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let info = TokenInfo::decode(self.conn).await?;
        event!(Level::INFO, "{}", info.message);
        Ok(ReceivedToken::Info(info))
    }

    async fn get_login_ack(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_feature_ext_ack(&mut self) -> crate::tabby::Result<ReceivedToken> {
    ...
    async fn get_sspi(&mut self) -> crate::tabby::Result<ReceivedToken> {
        let sspi = TokenSspi::decode_async(self.conn).await?;
        event!(Level::TRACE, "SSPI response");
        Ok(ReceivedToken::Sspi(sspi))
    }

    pub fn try_unfold(self) -> BoxStream<'a, crate::tabby::Result<ReceivedToken>> {
