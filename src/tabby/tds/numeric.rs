...
use crate::tabby::{sql_read_bytes::SqlReadBytes, Error};
...
impl Numeric {
    ...
    pub(crate) async fn decode<R>(src: &mut R, scale: u8) -> crate::tabby::Result<Option<Self>>
    where
...
impl Display for Numeric {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), fmt::Error> {
        if self.value < 0 {
            ...
            if self.scale > 0 {
                write!(f, "-{}.{:0pad$}", int_part, dec_part, pad = self.scale as usize)
            } else {
                write!(f, "-{}", int_part)
            }
...
mod tests {
    ...
    fn no_overflowing_pow() {
        use crate::tabby::{ColumnData, ToSql};
