...
use crate::tabby::client::{tls::TlsPreloginWrapper, tls_stream::create_tls_stream};
use crate::tabby::{
...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Connection<S> {
    ...
    pub(crate) async fn connect(config: Config, tcp_stream: S) -> crate::tabby::Result<Connection<S>> {
        let context = {
    ...
    async fn flush_done(&mut self) -> crate::tabby::Result<TokenDone> {
        TokenStream::new(self).flush_done().await
    }

    #[cfg(any(windows, feature = "integrated-auth-gssapi"))]
    /// Flush the incoming token stream until receiving `SSPI` token.
    async fn flush_sspi(&mut self) -> crate::tabby::Result<TokenSspi> {
    ...
    pub async fn send<E>(&mut self, mut header: PacketHeader, item: E) -> crate::tabby::Result<()>
    where
    ...
    pub(crate) async fn flush_sink(&mut self) -> crate::tabby::Result<()> {
    ...
    pub async fn flush_stream(&mut self) -> crate::tabby::Result<()> {
        self.buf.truncate(0);
    ...
    ) -> crate::tabby::Result<PreloginMessage> {
    ...
    async fn tls_handshake(self, _: &Config, _: EncryptionLevel) -> crate::tabby::Result<Self> {
        event!(
            Level::WARN,
            "TLS encryption is not enabled. All traffic including the login credentials are not encrypted."
        );
    ...
    pub(crate) async fn close(mut self) -> crate::tabby::Result<()> {
        self.transport.close().await
    }
}

impl<S: AsyncRead + AsyncWrite + Unpin + Send> Stream for Connection<S> {
    type Item = crate::tabby::Result<Packet>;
