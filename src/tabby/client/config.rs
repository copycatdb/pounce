...
impl Config {
    ...
    ///
    /// [ADO.NET connection string]: https://docs.microsoft.com/en-us/dotnet/framework/data/adonet/connection-strings
    pub fn from_ado_string(s: &str) -> crate::tabby::Result<Self> {
    ...
    pub fn from_jdbc_string(s: &str) -> crate::tabby::Result<Self> {
        let jdbc: JdbcConfig = s.parse()?;
        Self::from_config_string(jdbc)
    }

    fn from_config_string(s: impl ConfigString) -> crate::tabby::Result<Self> {
...
pub(crate) trait ConfigString {
    ...
    fn authentication(&self) -> crate::tabby::Result<AuthMethod> {
        let user = self
            .dict()
            .get("uid")
    ...
    fn trust_cert(&self) -> crate::tabby::Result<bool> {
        self.dict()
    ...
    fn encrypt(&self) -> crate::tabby::Result<EncryptionLevel> {
        self.dict()
            .get("encrypt")
    ...
    fn parse_bool<T: AsRef<str>>(v: T) -> crate::tabby::Result<bool> {
        match v.as_ref().trim().to_lowercase().as_str() {
            "true" | "yes" => Ok(true),
            "false" | "no" => Ok(false),
            _ => Err(crate::tabby::Error::Conversion(
