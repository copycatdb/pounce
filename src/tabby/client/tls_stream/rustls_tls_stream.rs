...
impl From<tokio_rustls::rustls::Error> for Error {
    fn from(e: tokio_rustls::rustls::Error) -> Self {
        crate::tabby::Error::Tls(e.to_string())
    }
}
...
fn get_server_name(config: &Config) -> crate::tabby::Result<ServerName<'static>> {
    match (ServerName::try_from(config.get_host().to_string()), &config.trust) {
        ...
        (Err(e), _) => Err(crate::tabby::Error::Tls(format!("{:?}", e))),
    }
}

impl<S: AsyncRead + AsyncWrite + Unpin + Send> TlsStream<S> {
    pub(super) async fn new(config: &Config, stream: S) -> crate::tabby::Result<Self> {
            ...
            TrustConfig::CaCertificateLocation(path) => {
                if let Ok(buf) = fs::read(path) {
                    let certs: Vec<CertificateDer<'static>> = match path.extension() {
                        ...
                        Some(_) | None => {
                            return Err(crate::tabby::Error::Io {
                    ...
                    for cert in certs {
                        cert_store.add(cert).map_err(|e| crate::tabby::Error::Tls(e.to_string()))?;
                    }
