...
///
/// ```no_run
/// # use crate::tabby::{Config, AuthMethod};
...
/// let client = crate::tabby::Client::connect(config, tcp.compat_write()).await?;
/// # Ok(())
/// # }
/// ```
///
...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Client<S> {
    ...
    ///
    /// [`Config`]: struct.Config.html
    pub async fn connect(config: Config, tcp_stream: S) -> crate::tabby::Result<Client<S>> {
        Ok(Client {
    ...
    pub async fn execute<'a>(
        ...
        params: &[&dyn ToSql],
    ) -> crate::tabby::Result<ExecuteResult> {
    ...
    ) -> crate::tabby::Result<QueryStream<'a>>
    where
        'a: 'b,
    {
        self.connection.flush_stream().await?;
    ...
    ///
    /// ```
    /// # use crate::tabby::{Config, IntoRow};
    /// # use tokio_util::compat::TokioAsyncWriteCompatExt;
    /// # use std::env;
    /// # #[tokio::main]
    /// # async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn bulk_insert<'a>(
        &'a mut self,
        table: &'a str,
    ) -> crate::tabby::Result<BulkLoadRequest<'a, S>> {
        ...
        let columns: Vec<_> = columns
            .ok_or_else(|| {
                crate::tabby::Error::Protocol("expecting column metadata from query but not found".into())
            })?
            .into_iter()
    ...
    pub async fn close(self) -> crate::tabby::Result<()> {
