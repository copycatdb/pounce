...
impl<'a> Query<'a> {
    ...
    pub async fn execute<'b, S>(self, client: &'b mut Client<S>) -> crate::tabby::Result<ExecuteResult>
    where
    ...
    pub async fn query<'b, S>(self, client: &'b mut Client<S>) -> crate::tabby::Result<QueryStream<'b>>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
        client.connection.flush_stream().await?;
